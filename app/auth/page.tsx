...
export default function AuthPage() {
  ...
  function google() {
    // Мок: Google подтверждает почту сразу — переходим к личным данным
    if (mode === "login") {
      router.push("/dashboard");
      return;
    }
    setViaGoogle(true);
    setEmail("aigerim@gmail.com");
    setStep(3);
  }
          ...
          {mode === "login" ? (
            <>
              ...
              <form onSubmit={login} className="space-y-3">
                <input required type="email" placeholder="Электронная почта" className={inputCls} />
                <input required type="password" placeholder="Пароль" className={inputCls} />
                <button
                  type="submit"
              ...
              {step === 2 && (
                <form onSubmit={confirmCode} className="space-y-4">
                  <p className="text-sm text-stone-600">
                    ...
                    <span className="font-medium text-stone-800">{email || "вашу почту"}</span>.
                ...
                <form onSubmit={finish} className="space-y-3">
                  {viaGoogle && (
                    <p className="rounded-xl bg-teal-50 px-4 py-2.5 text-xs text-teal-700">
                      Почта {email} подтверждена через Google
                    </p>
                  )}
